...
// We'll use this array to generate all of the options we need on our add and edit pages. 
$provincial_abbr = [
    "AB" => "Alberta",
    "BC" => "British Columbia",
    "MB" => "Manitoba",
...
function validate_city_input($city_name, $province, $population, $capital, $trivia, $provincial_abbr)
{
    ...
    if (isset($capital)) {
        if ($capital !== '1' && $capital !== '0') {
            ...
            // Convert the value to a boolean: true if '1', false if '0'. Technically, a BOOL is a TINYINT, constrained to a 0 or 1.
            $validated_data['capital'] = ($capital === '1');
        }
    } else {
        // We'll default to 0 (this will be handled as false) if no selection is made.
    ...
    if (!empty($trivia)) {
        ...
        // We can use mysqli_real_escape_string to help sanitise input before database insertion. Because this is part of the mysqli library / API, it needs a connection to the database to work.
